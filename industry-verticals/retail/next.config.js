...
const nextConfig = {
  ...
  async rewrites() {
    return {
      // beforeFiles rewrites are checked after headers/redirects and before all files including _next/public files
      // This allows static HTML files in public/ to be served directly
      beforeFiles: [
        // Serve static HTML files directly from public folder (bypass catch-all route)
        {
          source: '/:path*.html',
          destination: '/:path*.html',
        },
        // Handle locale-prefixed HTML file requests (redirect to non-prefixed version)
        {
          source: '/:locale(en|fr-FR|es-ES)/:path*.html',
          destination: '/:path*.html',
        },
      ],
